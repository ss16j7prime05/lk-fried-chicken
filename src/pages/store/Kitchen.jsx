...
export function Kitchen() {
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-gray-50 flex flex-col">
      {/* Header */}
      <div className="flex items-center justify-between px-4 md:px-6 py-4 bg-white border-b border-gray-100 flex-shrink-0">
        <div className="flex items-center gap-3">
          {!fullscreen && (
            <Link
              to="/store/v2/orders"
              className="p-3 rounded-xl bg-white border border-gray-200 text-gray-500 hover:bg-gray-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
          ...
          <div>
            ...
            <p className="text-[11px] font-medium text-gray-400">Live order board</p>
          </div>
        </div>

        <div className="flex items-center gap-2 md:gap-3">
          {/* Volume slider — md+ only */}
          {!muted && (
          ...
          <button
            onClick={handleMute}
            className={`p-3 rounded-xl border text-sm font-bold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary
              ${muted ? "border-orange-200 bg-orange-50 text-orange-500" : "border-gray-200 text-gray-500 hover:bg-gray-50"}`}
            aria-label={muted ? "Unmute kitchen sound" : "Mute kitchen sound"}
          >
            {muted ? <VolumeX size={20} /> : <Bell size={20} />}
          </button>

          {/* Fullscreen */}
          <button
            onClick={toggleFullscreen}
            className="p-3 rounded-xl border border-gray-200 text-gray-500 hover:bg-gray-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
          ...
          {muted && (
            <span className="hidden sm:flex items-center gap-1.5 text-xs font-bold text-orange-500 bg-orange-50 px-3 py-2 rounded-xl border border-orange-100">
